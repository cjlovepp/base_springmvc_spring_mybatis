...
public class SystemLogger {
	...
	/*private void initDBLogger() {
		try {
			String filePath = GlobleValue.DB_LOG + "db.txt";
			File fileLog = new File(filePath);
			// 20070806 System.out.println("fileLog.isExist:" +
			// fileLog.exists());
			if (!fileLog.exists()) {
				fileLog.createNewFile();
			}
			PatternLayout layout = new PatternLayout(
					"%-4r %-5p %3x %d{yyyy-MM-dd HH:mm:ss} | %m%n");
			DailyRollingFileAppender appender = new DailyRollingFileAppender(
					layout, filePath, "'.'yyyy-ww");
			((Log4JLogger) dbLogger).getLogger().addAppender(appender);
		} catch (IOException ex) {
			ex.printStackTrace();
		}
	}

	private void initOperationLogger() {
		try {
			String filePath = GlobleValue.OPERATION_LOG + "oa.txt";
			File fileLog = new File(filePath);
			if (!fileLog.exists()) {
				fileLog.createNewFile();
			}
			...
			DailyRollingFileAppender appender = new DailyRollingFileAppender(
					layout, filePath, "'.'yyyy-ww");
			((Log4JLogger) operationLogger).getLogger().addAppender(appender);
		} catch (IOException ex) {
			ex.printStackTrace();
		}
	}
	...
	private void initErrorLogger() {
		try {
			String filePath = GlobleValue.ERROR_LOG + "error.txt";
			File fileLog = new File(filePath);
			// 20070806 System.out.println("fileLog.isExist:" +
			// fileLog.exists());
			if (!fileLog.exists()) {
			...
			org.apache.log4j.Logger log = ((Log4JLogger) errorLogger)
					.getLogger();
			log.addAppender(appender);
			log.addAppender(conAppender);
